function [ a, cb ] = mySurfStatView( struct, surf, title, background);
...
if ~isstruct(struct)
    [a,cb]=mySurfStatViewData(struct,surf,title,background);
    ...
    if isfield(struct,'P')
        ...
        trisurf(surf.tri, surf.coord(1,:)', surf.coord(2,:)', surf.coord(3,:)', double(signifclus)- 2*double(signifpeak));
        a=find(signifclus>0);
        hold on; plot3(surf.coord(1,a), surf.coord(2,a), surf.coord(3,a), 'ro', 'MarkerSize', 6)
        colorbar
 
         
        figure; trisurf(surf.tri, surf.coord(1,:)', surf.coord(2,:)', surf.coord(3,:)',  struct.C);
         
         
        figure
        [a,cb]=mySurfStatViewData(tt,surf,title,background);
        cm=[zeros(1,3);
    ...
    set(cb,'Position',[0.1 0.08 0.7 0.03]);
 
end
 
h_xlabel = get(gca,'XLabel');
set(h_xlabel,'FontSize',6); 
