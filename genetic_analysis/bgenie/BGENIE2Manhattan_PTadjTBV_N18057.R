...
phenos2test<-c("totalBV",phenos2test)




for (phenotype in phenos2test){
#phenotype="AI_VOLUME_Insular_Cortex" # "AI_VOLUME_Planum_Temporale"
...
pattern1=paste(pheno_root,subset_name,"bgenie",type,region,"chr",sep="_")
...
for (file in files) {
  ...
  cat(as.character(Sys.time()),'\n')
  cat('Number of SNPs in chromsome ',chrom,':',NROW(d0),'\n')
  # check filters:
  # maf
...
# rename columns
colnames(data)<-gsub("^_","",gsub(gsub("_totalBV","",pheno,""),"",gsub("residuals_","",colnames(data))))
