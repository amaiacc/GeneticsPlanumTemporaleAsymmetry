...
for p in ${phenos}
 ...
 then
  ...
  if [ ! -f ${p_file}_h2_baselineLD.log ]
   then
   #---------------
...
for p1 in ${phenos}
 do
 p1_file=${working_dir}${p1}.sumstats.gz
 if [ ! -f ${p1}_GWASsumstats.log ]
 then
...
# then
  ...
  ldsc.py \
   ...
   --ref-ld-chr ${ldscores_dir}/eur_w_ld_chr/ \
   --w-ld-chr ${ldscores_dir}/eur_w_ld_chr/ \
   --out ${p1}_ALSPACsumstats_int0 --intercept-gencov 0,0,0,0,0,0
#  fi
 done
grep -A5 p2 *ALSPAC*log | grep -v Analysis | grep -e p2 -e sumstats | grep -v 'log-$' > summary_p1_p2_ALSPACgencor_ldsc.table
