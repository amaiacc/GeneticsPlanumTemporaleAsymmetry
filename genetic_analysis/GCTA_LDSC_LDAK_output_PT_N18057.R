...
t_comp_barplot<-ggplot(data=t2,color="black",aes(x=measure,y=h2,width=0.7,fill=program)) +  #fill=program
...
t_comp_barplot2<- t_comp_barplot 

ggsave(t_comp_barplot2,file=paste(out_dir,"h2_estimates_comparison_barplot_adjTBV.pdf",sep=""),width=10,height=7)
rm(t,t_comp_barplot,t_comp_barplot2)


write.csv(t3,paste(out_dir,"/estimates4power.csv",sep=""),row.names=TRUE)
