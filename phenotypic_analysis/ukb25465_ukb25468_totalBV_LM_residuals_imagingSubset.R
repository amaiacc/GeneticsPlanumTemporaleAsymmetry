...
covs_order<-c("age","zage2","sex",paste("PC",1:10,sep=""),"array",scanner_position ) #, "totalBV","height","BMI") "assessment_centre", 
...
t=4
length(c(pheno_tBV)) # 1221 measures (T1 volume + dti (FA skeleton and tracts)); AI,left, right)
...
f_tBV<-paste(out_dir,paste("/summary_phenotypes/",release,"_imaging",covs_name,"_totalBVPhenotypes_residuals_wHeader.table",sep=""),sep="")
write.table(data_s_tBV,file=f_tBV,sep="\t",quote=FALSE,row.names = FALSE,col.names=TRUE)
write.table(colnames(data_s_tBV),gsub("_wHeader.table",".list",f_tBV),col.names = FALSE,row.names = FALSE,quote=FALSE)
# rm(f_tBV,data_s_tBV)

# clean all
# rm(data_s)
# save image
save.image(paste(out_dir,release,"_LM_totalBV_residuals_imagingSubset",covs_name,".RData",sep=""))
